...
function Invoke-GraphRefresh {
    $GraphBuilder = Join-Path $RepoRoot "tools\\build_obsidian_graph_layer.py"
    Write-Host "Running build_obsidian_graph_layer.py"
    & $VenvPython $GraphBuilder --project-root $RepoRoot | Out-Host
    if ($LASTEXITCODE -ne 0) {
        throw "graph-refresh failed with exit code $LASTEXITCODE."
    }
}
...
switch ($Command) {
    ...
    "generated-surfaces" { Invoke-HarnessScript "check_generated_surfaces.py" }
    "schema-validate" {
    ...
    "graph-refresh" { Invoke-GraphRefresh }
    "source-audit" {
    ...
    "verify" {
        & $VenvPython -m wiki_obsidian.cli verify | Out-Host
        if ($LASTEXITCODE -ne 0) {
