---
aliases: []
attachment_bucket: null
attachment_section: Annex 10
basis: '3.1.6.2.

  Tractors with laden semi-trailer. A laden combination is understood to be a

  tractor in running order, with the driver on board, coupled to a laden semi-trailer.

  The dynamic load of the semi-trailer on the tractor shall be represented by a static

  mass Ps mounted at the fifth wheel coupling equal to:

  Ps = Pso (1 + 0.45z)

  where:

  Pso

  represents the difference between the maximum laden mass of the tractor

  and its unladen mass.

  For h the following value shall be taken:

  where:

  ho

  is the height of the centre of gravity of the tractor;

  hs

  is the height of the coupling on which the semi-trailer rests;

  Po

  is the unladen mass of the tractor alone

  and:'
browse_buckets: []
clause_path: annex-10-3-1-6-2
comparison_key: 849fb252bc9dce8d35681b15504a120cc2e440f3
confidence: medium
created: '2026-04-13'
document_id: pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english
document_kind: annex_clause
effective_date: null
functional_domain:
- other_or_review
id: regunit-pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english-annex-10-3-1-6-2
inherits_section_context: false
inherits_subject_context: false
is_attachment: false
is_table_like_row: false
jurisdiction: UNECE
legacy_domain: needs_review
line_index_end: null
line_index_start: null
note_type: regulation_unit
page_end: 112
page_start: 112
parent_clause_path: null
parent_clause_text: null
parent_title: ECE R13 UN Regulation No. 13 - Rev.6 - Heavy vehicle braking Rev0 English
phase: cross_phase
primary_topic: 3-1-6-2
provenance:
  parser_run_id: 20260414T051508Z__014a8f04
  source_files:
  - raw/collections/pdf_ece/ECE_R13_UN_Regulation_No._13_-_Rev.6_-_Heavy_vehicle_braking_Rev0_English.pdf
  source_hashes:
    raw/collections/pdf_ece/ECE_R13_UN_Regulation_No._13_-_Rev.6_-_Heavy_vehicle_braking_Rev0_English.pdf: 87e6376b1dc6d24435554a12133b2f78d9a80c255c028a6e45a84b6c7c3aa047
  source_url: null
raw_marker: annex-10-3.1.6.2
record_layer: knowledge
reference_articles: []
regulatory_layer: technical_requirement
review_required: true
row_group_id: null
secondary_topics: []
sectno: null
source_citation: pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english
  / annex-10-3-1-6-2
source_collection: pdf_ece
source_url: null
statement: 3.1.6.2.
status: draft
subject: null
summary: Regulation unit `annex-10-3-1-6-2` from pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english.
title: 3.1.6.2.
updated: '2026-04-13'
---

# 3.1.6.2.

## Statement
3.1.6.2.

## Classification
- jurisdiction: UNECE
- source_collection: pdf_ece
- regulatory_layer: technical_requirement
- phase: cross_phase
- functional_domain: other_or_review
- primary_topic: 3-1-6-2
- secondary_topics: n/a
- browse_buckets: n/a
- legacy_domain: needs_review

## Basis
3.1.6.2.
Tractors with laden semi-trailer. A laden combination is understood to be a
tractor in running order, with the driver on board, coupled to a laden semi-trailer.
The dynamic load of the semi-trailer on the tractor shall be represented by a static
mass Ps mounted at the fifth wheel coupling equal to:
Ps = Pso (1 + 0.45z)
where:
Pso
represents the difference between the maximum laden mass of the tractor
and its unladen mass.
For h the following value shall be taken:
where:
ho
is the height of the centre of gravity of the tractor;
hs
is the height of the coupling on which the semi-trailer rests;
Po
is the unladen mass of the tractor alone
and:

## Authority
- clause_path: annex-10-3-1-6-2
- source_file: raw/collections/pdf_ece/ECE_R13_UN_Regulation_No._13_-_Rev.6_-_Heavy_vehicle_braking_Rev0_English.pdf
- source_citation: pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english / annex-10-3-1-6-2
- source_url: n/a
- confidence: medium

## Related Notes
- [[regulation_documents/pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english]]
- [[jurisdictions/jurisdiction-unece]]
