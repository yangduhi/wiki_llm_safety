---
aliases: []
attachment_bucket: null
attachment_section: null
basis: (f) Measure the horizontal distance (D2) between Plane C and Plane D.
browse_buckets:
- occupant_restraints
clause_path: f-4
comparison_key: 10f30282031bf37a6ea9b2995cf5e3c84670cb74
confidence: medium
created: '2026-04-13'
document_id: xml_fmvss-571-201
document_kind: null
effective_date: null
functional_domain:
- crash_avoidance_and_vehicle_control
id: regunit-xml_fmvss-571-201-f-4
inherits_section_context: false
inherits_subject_context: false
is_attachment: false
is_table_like_row: false
jurisdiction: US
legacy_domain: passive_crash
line_index_end: null
line_index_start: null
note_type: regulation_unit
page_end: null
page_start: null
parent_clause_path: null
parent_clause_text: null
parent_title: § 571.201 Standard No. 201; Occupant protection in interior impact.
phase: in_crash
primary_topic: f-measure-the-horizontal-distance-d2-between-plane-c-and-plane-d
provenance:
  parser_run_id: 20260414T050834Z__953d9693
  source_files:
  - raw/collections/xml_fmvss/571.201.xml
  source_hashes:
    raw/collections/xml_fmvss/571.201.xml: 3d6892280e12b52cee800b97b7828f6f5913b18fb5f64dc19f61210284212ac8
  source_url: null
raw_marker: (f)
record_layer: knowledge
reference_articles: []
regulatory_layer: technical_requirement
review_required: false
row_group_id: null
secondary_topics: []
sectno: null
source_citation: xml_fmvss-571-201 / f-4
source_collection: xml_fmvss
source_url: null
statement: (f) Measure the horizontal distance (D2) between Plane C and Plane D.
status: draft
subject: null
summary: Regulation unit `f-4` from xml_fmvss-571-201.
title: (f) Measure the horizontal distance (D2) between Plane C and Plane D.
updated: '2026-04-13'
---

# (f) Measure the horizontal distance (D2) between Plane C and Plane D.

## Statement
(f) Measure the horizontal distance (D2) between Plane C and Plane D.

## Classification
- jurisdiction: US
- source_collection: xml_fmvss
- regulatory_layer: technical_requirement
- phase: in_crash
- functional_domain: crash_avoidance_and_vehicle_control
- primary_topic: f-measure-the-horizontal-distance-d2-between-plane-c-and-plane-d
- secondary_topics: n/a
- browse_buckets: occupant_restraints
- legacy_domain: passive_crash

## Basis
(f) Measure the horizontal distance (D2) between Plane C and Plane D.

## Authority
- clause_path: f-4
- source_file: raw/collections/xml_fmvss/571.201.xml
- source_citation: xml_fmvss-571-201 / f-4
- source_url: n/a
- confidence: medium

## Related Notes
- [[regulation_documents/xml_fmvss-571-201]]
- [[jurisdictions/jurisdiction-us]]
