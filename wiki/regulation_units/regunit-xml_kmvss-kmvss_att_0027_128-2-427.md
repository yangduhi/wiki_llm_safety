---
aliases: []
attachment_bucket: short_value_rows
attachment_section: 부분 자율주행시스템의 안전기준(제111조의3 관련)
basis: '2) 위 그림에서 전방감지거리(전방의 자동차 및 사람 등을 감지하는 거리를 말

  한다) 및 측방감지거리(측방의 자동차를 감지하는 거리를 말한다)는 다음의

  구분에 따른 기준을 충족할 것'
browse_buckets: []
clause_path: 2-427
comparison_key: b0bc5ba90f96112a07158f4b25e777e831300598
confidence: medium
created: '2026-04-13'
document_id: xml_kmvss-kmvss_att_0027_128
document_kind: attachment
effective_date: null
functional_domain:
- crash_avoidance_and_vehicle_control
id: regunit-xml_kmvss-kmvss_att_0027_128-2-427
inherits_section_context: true
inherits_subject_context: true
is_attachment: true
is_table_like_row: false
jurisdiction: KR
legacy_domain: active_safety
line_index_end: 431
line_index_start: 427
note_type: regulation_unit
page_end: null
page_start: null
parent_clause_path: null
parent_clause_text: null
parent_title: 2) 위 그림에서 전방감지거리(전방의 자동차 및 사람 등을 감지하는 거리를 말
phase: pre_crash
primary_topic: automated_driving_assist
provenance:
  parser_run_id: 20260414T051134Z__dc745dc5
  source_files:
  - raw/collections/xml_kmvss/KMVSS_Att_0027_128.xml
  source_hashes:
    raw/collections/xml_kmvss/KMVSS_Att_0027_128.xml: b71721534f9d4b9153ee3a120c87da2e70731a36cb5ca10655eb5db604c4674a
  source_url: null
raw_marker: 2-427
record_layer: knowledge
reference_articles:
- 제111조의3
regulatory_layer: technical_requirement
review_required: true
row_group_id: group-52
secondary_topics:
- automated_driving
sectno: 별표 0027
source_citation: xml_kmvss-kmvss_att_0027_128 / 2-427
source_collection: xml_kmvss
source_url: null
statement: 2) 위 그림에서 전방감지거리(전방의 자동차 및 사람 등을 감지하는 거리를 말
status: draft
subject: 부분 자율주행시스템의 안전기준(제111조의3 관련)
summary: Regulation unit `2-427` from xml_kmvss-kmvss_att_0027_128.
title: 2) 위 그림에서 전방감지거리(전방의 자동차 및 사람 등을 감지하는 거리를 말
updated: '2026-04-13'
---

# 2) 위 그림에서 전방감지거리(전방의 자동차 및 사람 등을 감지하는 거리를 말

## Statement
2) 위 그림에서 전방감지거리(전방의 자동차 및 사람 등을 감지하는 거리를 말

## Classification
- jurisdiction: KR
- source_collection: xml_kmvss
- regulatory_layer: technical_requirement
- phase: pre_crash
- functional_domain: crash_avoidance_and_vehicle_control
- primary_topic: automated_driving_assist
- secondary_topics: automated_driving
- browse_buckets: n/a
- legacy_domain: active_safety

## Basis
2) 위 그림에서 전방감지거리(전방의 자동차 및 사람 등을 감지하는 거리를 말
한다) 및 측방감지거리(측방의 자동차를 감지하는 거리를 말한다)는 다음의
구분에 따른 기준을 충족할 것

## Authority
- clause_path: 2-427
- source_file: raw/collections/xml_kmvss/KMVSS_Att_0027_128.xml
- source_citation: xml_kmvss-kmvss_att_0027_128 / 2-427
- source_url: n/a
- confidence: medium

## Related Notes
- [[regulation_documents/xml_kmvss-kmvss_att_0027_128]]
- [[jurisdictions/jurisdiction-kr]]
