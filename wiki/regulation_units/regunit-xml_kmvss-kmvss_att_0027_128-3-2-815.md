---
aliases: []
attachment_bucket: short_value_rows
attachment_section: 부분 자율주행시스템의 안전기준(제111조의3 관련)
basis: '3) 위 2) 단서에 따라 다른 자동차, 장애물 등을 회피하게 하는 경우 자동차 앞

  바퀴 타이어의 바깥쪽 접지부분 가장자리가 주행 중인 차선의 바깥쪽 경계를

  넘지 않아야 하며, 회피하는 동작이 종료된 후에는 자동차를 안정된 위치로

  최대한 빨리 복귀시킬 것'
browse_buckets: []
clause_path: 3-2-815
comparison_key: 02841bace9af2982139062e92e22051a8773cab6
confidence: medium
created: '2026-04-13'
document_id: xml_kmvss-kmvss_att_0027_128
document_kind: attachment
effective_date: null
functional_domain:
- crash_avoidance_and_vehicle_control
id: regunit-xml_kmvss-kmvss_att_0027_128-3-2-815
inherits_section_context: true
inherits_subject_context: true
is_attachment: true
is_table_like_row: false
jurisdiction: KR
legacy_domain: active_safety
line_index_end: 821
line_index_start: 815
note_type: regulation_unit
page_end: null
page_start: null
parent_clause_path: null
parent_clause_text: null
parent_title: 3) 위 2) 단서에 따라 다른 자동차, 장애물 등을 회피하게 하는 경우 자동차 앞
phase: pre_crash
primary_topic: automated_driving_assist
provenance:
  parser_run_id: 20260414T051134Z__dc745dc5
  source_files:
  - raw/collections/xml_kmvss/KMVSS_Att_0027_128.xml
  source_hashes:
    raw/collections/xml_kmvss/KMVSS_Att_0027_128.xml: b71721534f9d4b9153ee3a120c87da2e70731a36cb5ca10655eb5db604c4674a
  source_url: null
raw_marker: 3-2-815
record_layer: knowledge
reference_articles:
- 제111조의3
regulatory_layer: technical_requirement
review_required: true
row_group_id: group-102
secondary_topics:
- automated_driving
sectno: 별표 0027
source_citation: xml_kmvss-kmvss_att_0027_128 / 3-2-815
source_collection: xml_kmvss
source_url: null
statement: 3) 위 2) 단서에 따라 다른 자동차, 장애물 등을 회피하게 하는 경우 자동차 앞
status: draft
subject: 부분 자율주행시스템의 안전기준(제111조의3 관련)
summary: Regulation unit `3-2-815` from xml_kmvss-kmvss_att_0027_128.
title: 3) 위 2) 단서에 따라 다른 자동차, 장애물 등을 회피하게 하는 경우 자동차 앞
updated: '2026-04-13'
---

# 3) 위 2) 단서에 따라 다른 자동차, 장애물 등을 회피하게 하는 경우 자동차 앞

## Statement
3) 위 2) 단서에 따라 다른 자동차, 장애물 등을 회피하게 하는 경우 자동차 앞

## Classification
- jurisdiction: KR
- source_collection: xml_kmvss
- regulatory_layer: technical_requirement
- phase: pre_crash
- functional_domain: crash_avoidance_and_vehicle_control
- primary_topic: automated_driving_assist
- secondary_topics: automated_driving
- browse_buckets: n/a
- legacy_domain: active_safety

## Basis
3) 위 2) 단서에 따라 다른 자동차, 장애물 등을 회피하게 하는 경우 자동차 앞
바퀴 타이어의 바깥쪽 접지부분 가장자리가 주행 중인 차선의 바깥쪽 경계를
넘지 않아야 하며, 회피하는 동작이 종료된 후에는 자동차를 안정된 위치로
최대한 빨리 복귀시킬 것

## Authority
- clause_path: 3-2-815
- source_file: raw/collections/xml_kmvss/KMVSS_Att_0027_128.xml
- source_citation: xml_kmvss-kmvss_att_0027_128 / 3-2-815
- source_url: n/a
- confidence: medium

## Related Notes
- [[regulation_documents/xml_kmvss-kmvss_att_0027_128]]
- [[jurisdictions/jurisdiction-kr]]
