---
aliases: []
attachment_bucket: null
attachment_section: null
basis: (e) Label on the dashboard. (1) Except as provided in S4.5.1(e)(2) or S4.5.1(e)(3),
  each vehicle that is equipped with an inflatable restraint for the passenger position
  shall have a label attached to a location on the dashboard or the steering control
  hub that is clearly visible from all front seating positions. The label need not
  be permanently affixed to the vehicle. This label shall conform in content to the
  label shown in Figure 7 of this standard, and shall comply with the requirements
  of S4.5.1(e)(1)(i) through S4.5.1(e)(1)(iii).
browse_buckets:
- occupant_restraints
- seat_systems
- steering_control
clause_path: e-2
comparison_key: 7ef0ff805d86564fd562986afdd0eb4a0b36d568
confidence: medium
created: '2026-04-13'
document_id: xml_fmvss-571-208
document_kind: null
effective_date: null
functional_domain:
- occupant_protection_and_restraints
id: regunit-xml_fmvss-571-208-e-2
inherits_section_context: false
inherits_subject_context: false
is_attachment: false
is_table_like_row: false
jurisdiction: US
legacy_domain: passive_crash
line_index_end: null
line_index_start: null
note_type: regulation_unit
page_end: null
page_start: null
parent_clause_path: null
parent_clause_text: null
parent_title: § 571.208 Standard No. 208; Occupant crash protection.
phase: in_crash
primary_topic: frontal_impact
provenance:
  parser_run_id: 20260414T050834Z__953d9693
  source_files:
  - raw/collections/xml_fmvss/571.208.xml
  source_hashes:
    raw/collections/xml_fmvss/571.208.xml: ee70d156fea349dc14f70044beae0c658368630b26386f3ce3bdd989287dee60
  source_url: null
raw_marker: (e)
record_layer: knowledge
reference_articles: []
regulatory_layer: technical_requirement
review_required: false
row_group_id: null
secondary_topics: []
sectno: null
source_citation: xml_fmvss-571-208 / e-2
source_collection: xml_fmvss
source_url: null
statement: (e) Label on the dashboard. (1) Except as provided in S4.5.1(e)(2) or S4.5.1(e)(3),
  each vehicle that is equipped with an inflatable restraint for the passenger position
  shall have a label attached to a location on the dashboard or the steering control
  hub that is clearly visible from all front seating positions. The label need not
  be permanently affixed to the vehicle. This label shall conform in content to the
  label shown in Figure 7 of this standard, and shall comply with the requirements
  of S4.5.1(e)(1)(i) through S4.5.1(e)(1)(iii).
status: draft
subject: null
summary: Regulation unit `e-2` from xml_fmvss-571-208.
title: (e) Label on the dashboard. (1) Except as provided in S4.5.1(e)(2) or S4.5.1(e)(3),
  each vehicle that is equipped with a
updated: '2026-04-13'
---

# (e) Label on the dashboard. (1) Except as provided in S4.5.1(e)(2) or S4.5.1(e)(3), each vehicle that is equipped with a

## Statement
(e) Label on the dashboard. (1) Except as provided in S4.5.1(e)(2) or S4.5.1(e)(3), each vehicle that is equipped with an inflatable restraint for the passenger position shall have a label attached to a location on the dashboard or the steering control hub that is clearly visible from all front seating positions. The label need not be permanently affixed to the vehicle. This label shall conform in content to the label shown in Figure 7 of this standard, and shall comply with the requirements of S4.5.1(e)(1)(i) through S4.5.1(e)(1)(iii).

## Classification
- jurisdiction: US
- source_collection: xml_fmvss
- regulatory_layer: technical_requirement
- phase: in_crash
- functional_domain: occupant_protection_and_restraints
- primary_topic: frontal_impact
- secondary_topics: n/a
- browse_buckets: occupant_restraints, seat_systems, steering_control
- legacy_domain: passive_crash

## Basis
(e) Label on the dashboard. (1) Except as provided in S4.5.1(e)(2) or S4.5.1(e)(3), each vehicle that is equipped with an inflatable restraint for the passenger position shall have a label attached to a location on the dashboard or the steering control hub that is clearly visible from all front seating positions. The label need not be permanently affixed to the vehicle. This label shall conform in content to the label shown in Figure 7 of this standard, and shall comply with the requirements of S4.5.1(e)(1)(i) through S4.5.1(e)(1)(iii).

## Authority
- clause_path: e-2
- source_file: raw/collections/xml_fmvss/571.208.xml
- source_citation: xml_fmvss-571-208 / e-2
- source_url: n/a
- confidence: medium

## Related Notes
- [[regulation_documents/xml_fmvss-571-208]]
- [[jurisdictions/jurisdiction-us]]
