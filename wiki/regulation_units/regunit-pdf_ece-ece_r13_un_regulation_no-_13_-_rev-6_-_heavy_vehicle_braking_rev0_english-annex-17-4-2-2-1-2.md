---
aliases: []
attachment_bucket: null
attachment_section: Annex 17
basis: '4.2.2.1.2.

  For trailers equipped with only an electric control line, the response to messages

  defined in EBS 12 of ISO 11992-2:2003 shall be checked as follows:

  The pneumatic supply line at the start of each test shall be > 700 kPa.

  The electric control line shall be connected to the simulator.

  The simulator shall transmit the following messages:

  Byte 3, bits 5 - 6 of EBS 12 set to 01b to indicate to the trailer that a pneumatic

  control line is available.

  Byte 3-4 of EBS 11 shall be set to 0 (no service brake demand)

  The response to the following messages shall be checked:

  EBS 12, Byte 3,

  Bit 1-2

  Pressure in the brake chambers or reaction of the trailer

  01b

  0 kPa (service brake released)

  00b

  The trailer is automatically braked to demonstrate that the

  combination is not compatible. A signal should also be

  transmitted via Pin 5 of the ISO 7638:1997 connector (yellow

  warning).'
browse_buckets: []
clause_path: annex-17-4-2-2-1-2
comparison_key: 823563477e697aef39c46e68335ba70f8be8ad59
confidence: medium
created: '2026-04-13'
document_id: pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english
document_kind: annex_clause
effective_date: null
functional_domain:
- other_or_review
id: regunit-pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english-annex-17-4-2-2-1-2
inherits_section_context: false
inherits_subject_context: false
is_attachment: false
is_table_like_row: false
jurisdiction: UNECE
legacy_domain: needs_review
line_index_end: null
line_index_start: null
note_type: regulation_unit
page_end: 219
page_start: 219
parent_clause_path: null
parent_clause_text: null
parent_title: ECE R13 UN Regulation No. 13 - Rev.6 - Heavy vehicle braking Rev0 English
phase: cross_phase
primary_topic: 4-2-2-1-2
provenance:
  parser_run_id: 20260414T051508Z__014a8f04
  source_files:
  - raw/collections/pdf_ece/ECE_R13_UN_Regulation_No._13_-_Rev.6_-_Heavy_vehicle_braking_Rev0_English.pdf
  source_hashes:
    raw/collections/pdf_ece/ECE_R13_UN_Regulation_No._13_-_Rev.6_-_Heavy_vehicle_braking_Rev0_English.pdf: 87e6376b1dc6d24435554a12133b2f78d9a80c255c028a6e45a84b6c7c3aa047
  source_url: null
raw_marker: annex-17-4.2.2.1.2
record_layer: knowledge
reference_articles: []
regulatory_layer: technical_requirement
review_required: true
row_group_id: null
secondary_topics: []
sectno: null
source_citation: pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english
  / annex-17-4-2-2-1-2
source_collection: pdf_ece
source_url: null
statement: 4.2.2.1.2.
status: draft
subject: null
summary: Regulation unit `annex-17-4-2-2-1-2` from pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english.
title: 4.2.2.1.2.
updated: '2026-04-13'
---

# 4.2.2.1.2.

## Statement
4.2.2.1.2.

## Classification
- jurisdiction: UNECE
- source_collection: pdf_ece
- regulatory_layer: technical_requirement
- phase: cross_phase
- functional_domain: other_or_review
- primary_topic: 4-2-2-1-2
- secondary_topics: n/a
- browse_buckets: n/a
- legacy_domain: needs_review

## Basis
4.2.2.1.2.
For trailers equipped with only an electric control line, the response to messages
defined in EBS 12 of ISO 11992-2:2003 shall be checked as follows:
The pneumatic supply line at the start of each test shall be > 700 kPa.
The electric control line shall be connected to the simulator.
The simulator shall transmit the following messages:
Byte 3, bits 5 - 6 of EBS 12 set to 01b to indicate to the trailer that a pneumatic
control line is available.
Byte 3-4 of EBS 11 shall be set to 0 (no service brake demand)
The response to the following messages shall be checked:
EBS 12, Byte 3,
Bit 1-2
Pressure in the brake chambers or reaction of the trailer
01b
0 kPa (service brake released)
00b
The trailer is automatically braked to demonstrate that the
combination is not compatible. A signal should also be
transmitted via Pin 5 of the ISO 7638:1997 connector (yellow
warning).

## Authority
- clause_path: annex-17-4-2-2-1-2
- source_file: raw/collections/pdf_ece/ECE_R13_UN_Regulation_No._13_-_Rev.6_-_Heavy_vehicle_braking_Rev0_English.pdf
- source_citation: pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english / annex-17-4-2-2-1-2
- source_url: n/a
- confidence: medium

## Related Notes
- [[regulation_documents/pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english]]
- [[jurisdictions/jurisdiction-unece]]
