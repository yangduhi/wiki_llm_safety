---
aliases: []
attachment_bucket: null
attachment_section: Annex 7.
basis: '(a)

  The signal may be activated by the application of the service braking

  system in such a manner that it would produce, in an unladen condition

  and engine disconnected, under the test conditions of Type-0 as described

  in Annex 4, a deceleration as follows:

  Shall not be activated below

  N1

  6 m/s2

  M2, M3, N2 and N3

  4 m/s2

  The signal shall be de-activated for all vehicles at the latest when the

  deceleration has fallen below 2.5 m/s2.'
browse_buckets: []
clause_path: annex-7-a
comparison_key: 2b47d3f11c9be3b917d511daf847e8fc02a5796a
confidence: medium
created: '2026-04-13'
document_id: pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english
document_kind: annex_clause
effective_date: null
functional_domain:
- other_or_review
id: regunit-pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english-annex-7-a
inherits_section_context: false
inherits_subject_context: false
is_attachment: false
is_table_like_row: false
jurisdiction: UNECE
legacy_domain: needs_review
line_index_end: null
line_index_start: null
note_type: regulation_unit
page_end: 42
page_start: 42
parent_clause_path: null
parent_clause_text: null
parent_title: ECE R13 UN Regulation No. 13 - Rev.6 - Heavy vehicle braking Rev0 English
phase: cross_phase
primary_topic: a
provenance:
  parser_run_id: 20260414T051508Z__014a8f04
  source_files:
  - raw/collections/pdf_ece/ECE_R13_UN_Regulation_No._13_-_Rev.6_-_Heavy_vehicle_braking_Rev0_English.pdf
  source_hashes:
    raw/collections/pdf_ece/ECE_R13_UN_Regulation_No._13_-_Rev.6_-_Heavy_vehicle_braking_Rev0_English.pdf: 87e6376b1dc6d24435554a12133b2f78d9a80c255c028a6e45a84b6c7c3aa047
  source_url: null
raw_marker: annex-7-(a)
record_layer: knowledge
reference_articles: []
regulatory_layer: technical_requirement
review_required: true
row_group_id: null
secondary_topics: []
sectno: null
source_citation: pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english
  / annex-7-a
source_collection: pdf_ece
source_url: null
statement: (a)
status: draft
subject: null
summary: Regulation unit `annex-7-a` from pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english.
title: (a)
updated: '2026-04-13'
---

# (a)

## Statement
(a)

## Classification
- jurisdiction: UNECE
- source_collection: pdf_ece
- regulatory_layer: technical_requirement
- phase: cross_phase
- functional_domain: other_or_review
- primary_topic: a
- secondary_topics: n/a
- browse_buckets: n/a
- legacy_domain: needs_review

## Basis
(a)
The signal may be activated by the application of the service braking
system in such a manner that it would produce, in an unladen condition
and engine disconnected, under the test conditions of Type-0 as described
in Annex 4, a deceleration as follows:
Shall not be activated below
N1
6 m/s2
M2, M3, N2 and N3
4 m/s2
The signal shall be de-activated for all vehicles at the latest when the
deceleration has fallen below 2.5 m/s2.

## Authority
- clause_path: annex-7-a
- source_file: raw/collections/pdf_ece/ECE_R13_UN_Regulation_No._13_-_Rev.6_-_Heavy_vehicle_braking_Rev0_English.pdf
- source_citation: pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english / annex-7-a
- source_url: n/a
- confidence: medium

## Related Notes
- [[regulation_documents/pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english]]
- [[jurisdictions/jurisdiction-unece]]
