---
aliases: []
attachment_bucket: null
attachment_section: Annex 7.
basis: '5.2.1.28.5.

  Compensation by the coupling force control system shall be indicated by means

  of the yellow warning signal specified in paragraph 5.2.1.29.1.2. if this

  compensation exceeds 150 kPa away from the nominal demand value defined in

  paragraph 2.28.3. up to a limit, in pm, of 650 kPa (or the equivalent digital

  demand). Above the level of 650 kPa the warning shall be given if the

  compensation causes the operating point to lie outside the laden compatibility

  band as specified in Annex 10 for the motor vehicle.

  E/ECE/324

  E/ECE/TRANS/505

  } Rev.1/Add.12/Rev.6

  Regulation No. 13

  page 38

  Diagram 1

  Towing vehicles for trailers (except semi-trailers)

  Diagram 2

  Tractive units for semi-trailers

  0.8

  0.7

  0.6

  0.5

  0.4

  0.3

  0.2

  0.1

  Yellow

  Warning

  Signal

  Yellow

  Warning

  Signal

  Typical Nominal

  Demand line

  PM (kPa)

  T

  P

  M

  M

  0.8

  0.7

  0.6

  0.5

  0.4

  0.3

  0.2

  0.1

  150 kPa

  150 kPa

  Yellow

  Warning

  Signal

  Yellow

  Warning

  Signal

  Typical Nominal

  Demand Line

  PM (kPa)

  T

  P

  M

  M

  E/ECE/324

  E/ECE/TRANS/505 } Rev.1/Add.12/Rev.6

  Regulation No. 13

  page 39'
browse_buckets:
- side_impact
clause_path: annex-7-5-2-1-28-5
comparison_key: 1319cd05a65cae01e2e5175b698c0424cc60bdc9
confidence: medium
created: '2026-04-13'
document_id: pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english
document_kind: annex_clause
effective_date: null
functional_domain:
- other_or_review
id: regunit-pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english-annex-7-5-2-1-28-5
inherits_section_context: false
inherits_subject_context: false
is_attachment: false
is_table_like_row: false
jurisdiction: UNECE
legacy_domain: needs_review
line_index_end: null
line_index_start: null
note_type: regulation_unit
page_end: 39
page_start: 37
parent_clause_path: null
parent_clause_text: null
parent_title: ECE R13 UN Regulation No. 13 - Rev.6 - Heavy vehicle braking Rev0 English
phase: cross_phase
primary_topic: 5-2-1-28-5
provenance:
  parser_run_id: 20260414T051508Z__014a8f04
  source_files:
  - raw/collections/pdf_ece/ECE_R13_UN_Regulation_No._13_-_Rev.6_-_Heavy_vehicle_braking_Rev0_English.pdf
  source_hashes:
    raw/collections/pdf_ece/ECE_R13_UN_Regulation_No._13_-_Rev.6_-_Heavy_vehicle_braking_Rev0_English.pdf: 87e6376b1dc6d24435554a12133b2f78d9a80c255c028a6e45a84b6c7c3aa047
  source_url: null
raw_marker: annex-7-5.2.1.28.5
record_layer: knowledge
reference_articles: []
regulatory_layer: technical_requirement
review_required: true
row_group_id: null
secondary_topics: []
sectno: null
source_citation: pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english
  / annex-7-5-2-1-28-5
source_collection: pdf_ece
source_url: null
statement: 5.2.1.28.5.
status: draft
subject: null
summary: Regulation unit `annex-7-5-2-1-28-5` from pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english.
title: 5.2.1.28.5.
updated: '2026-04-13'
---

# 5.2.1.28.5.

## Statement
5.2.1.28.5.

## Classification
- jurisdiction: UNECE
- source_collection: pdf_ece
- regulatory_layer: technical_requirement
- phase: cross_phase
- functional_domain: other_or_review
- primary_topic: 5-2-1-28-5
- secondary_topics: n/a
- browse_buckets: side_impact
- legacy_domain: needs_review

## Basis
5.2.1.28.5.
Compensation by the coupling force control system shall be indicated by means
of the yellow warning signal specified in paragraph 5.2.1.29.1.2. if this
compensation exceeds 150 kPa away from the nominal demand value defined in
paragraph 2.28.3. up to a limit, in pm, of 650 kPa (or the equivalent digital
demand). Above the level of 650 kPa the warning shall be given if the
compensation causes the operating point to lie outside the laden compatibility
band as specified in Annex 10 for the motor vehicle.
E/ECE/324
E/ECE/TRANS/505
} Rev.1/Add.12/Rev.6
Regulation No. 13
page 38
Diagram 1
Towing vehicles for trailers (except semi-trailers)
Diagram 2
Tractive units for semi-trailers
0.8
0.7
0.6
0.5
0.4
0.3
0.2
0.1
Yellow
Warning
Signal
Yellow
Warning
Signal
Typical Nominal
Demand line
PM (kPa)
T
P
M
M
0.8
0.7
0.6
0.5
0.4
0.3
0.2
0.1
150 kPa
150 kPa
Yellow
Warning
Signal
Yellow
Warning
Signal
Typical Nominal
Demand Line
PM (kPa)
T
P
M
M
E/ECE/324
E/ECE/TRANS/505 } Rev.1/Add.12/Rev.6
Regulation No. 13
page 39

## Authority
- clause_path: annex-7-5-2-1-28-5
- source_file: raw/collections/pdf_ece/ECE_R13_UN_Regulation_No._13_-_Rev.6_-_Heavy_vehicle_braking_Rev0_English.pdf
- source_citation: pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english / annex-7-5-2-1-28-5
- source_url: n/a
- confidence: medium

## Related Notes
- [[regulation_documents/pdf_ece-ece_r13_un_regulation_no-_13_-_rev-6_-_heavy_vehicle_braking_rev0_english]]
- [[jurisdictions/jurisdiction-unece]]
