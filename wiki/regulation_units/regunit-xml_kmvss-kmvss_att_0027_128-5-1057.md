---
aliases: []
attachment_bucket: short_value_rows
attachment_section: 부분 자율주행시스템의 안전기준(제111조의3 관련)
basis: '5) 자율주행정보 기록장치에 기록된 정보는 법령에 따라 이를 열람할 수 있는

  자가 그 내용을 확인하는 것에 지장이 없거나 제작자가 열람방법을 제공하는

  방식으로 열람할 수 있어야 한다.'
browse_buckets: []
clause_path: 5-1057
comparison_key: 007a79dea4abf44678e978d67ee065b3961fa0cf
confidence: medium
created: '2026-04-13'
document_id: xml_kmvss-kmvss_att_0027_128
document_kind: attachment
effective_date: null
functional_domain:
- crash_avoidance_and_vehicle_control
id: regunit-xml_kmvss-kmvss_att_0027_128-5-1057
inherits_section_context: true
inherits_subject_context: true
is_attachment: true
is_table_like_row: false
jurisdiction: KR
legacy_domain: active_safety
line_index_end: 1061
line_index_start: 1057
note_type: regulation_unit
page_end: null
page_start: null
parent_clause_path: null
parent_clause_text: null
parent_title: 5) 자율주행정보 기록장치에 기록된 정보는 법령에 따라 이를 열람할 수 있는
phase: pre_crash
primary_topic: automated_driving_assist
provenance:
  parser_run_id: 20260414T051134Z__dc745dc5
  source_files:
  - raw/collections/xml_kmvss/KMVSS_Att_0027_128.xml
  source_hashes:
    raw/collections/xml_kmvss/KMVSS_Att_0027_128.xml: b71721534f9d4b9153ee3a120c87da2e70731a36cb5ca10655eb5db604c4674a
  source_url: null
raw_marker: 5-1057
record_layer: knowledge
reference_articles:
- 제111조의3
regulatory_layer: technical_requirement
review_required: true
row_group_id: group-164
secondary_topics:
- automated_driving
sectno: 별표 0027
source_citation: xml_kmvss-kmvss_att_0027_128 / 5-1057
source_collection: xml_kmvss
source_url: null
statement: 5) 자율주행정보 기록장치에 기록된 정보는 법령에 따라 이를 열람할 수 있는
status: draft
subject: 부분 자율주행시스템의 안전기준(제111조의3 관련)
summary: Regulation unit `5-1057` from xml_kmvss-kmvss_att_0027_128.
title: 5) 자율주행정보 기록장치에 기록된 정보는 법령에 따라 이를 열람할 수 있는
updated: '2026-04-13'
---

# 5) 자율주행정보 기록장치에 기록된 정보는 법령에 따라 이를 열람할 수 있는

## Statement
5) 자율주행정보 기록장치에 기록된 정보는 법령에 따라 이를 열람할 수 있는

## Classification
- jurisdiction: KR
- source_collection: xml_kmvss
- regulatory_layer: technical_requirement
- phase: pre_crash
- functional_domain: crash_avoidance_and_vehicle_control
- primary_topic: automated_driving_assist
- secondary_topics: automated_driving
- browse_buckets: n/a
- legacy_domain: active_safety

## Basis
5) 자율주행정보 기록장치에 기록된 정보는 법령에 따라 이를 열람할 수 있는
자가 그 내용을 확인하는 것에 지장이 없거나 제작자가 열람방법을 제공하는
방식으로 열람할 수 있어야 한다.

## Authority
- clause_path: 5-1057
- source_file: raw/collections/xml_kmvss/KMVSS_Att_0027_128.xml
- source_citation: xml_kmvss-kmvss_att_0027_128 / 5-1057
- source_url: n/a
- confidence: medium

## Related Notes
- [[regulation_documents/xml_kmvss-kmvss_att_0027_128]]
- [[jurisdictions/jurisdiction-kr]]
