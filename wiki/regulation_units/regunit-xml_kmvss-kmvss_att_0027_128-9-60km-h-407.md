---
aliases: []
attachment_bucket: short_value_rows
attachment_section: 부분 자율주행시스템의 안전기준(제111조의3 관련)
basis: '9) 작동상태에 있는 자동차로유지기능은 60km/h를 초과하는 속도로 주행 중인

  자동차의 전방에서 도로를 횡단 중인 보행자를 감지하면 즉시 충돌위험을 감

  소시키기 위한 형태의 운행이 시작되도록 할 것'
browse_buckets: []
clause_path: 9-60km-h-407
comparison_key: 50e2a3def6b5827d58ff44278c0b92fad254da76
confidence: medium
created: '2026-04-13'
document_id: xml_kmvss-kmvss_att_0027_128
document_kind: attachment
effective_date: null
functional_domain:
- crash_avoidance_and_vehicle_control
id: regunit-xml_kmvss-kmvss_att_0027_128-9-60km-h-407
inherits_section_context: true
inherits_subject_context: true
is_attachment: true
is_table_like_row: false
jurisdiction: KR
legacy_domain: active_safety
line_index_end: 411
line_index_start: 407
note_type: regulation_unit
page_end: null
page_start: null
parent_clause_path: null
parent_clause_text: null
parent_title: 9) 작동상태에 있는 자동차로유지기능은 60km/h를 초과하는 속도로 주행 중인
phase: pre_crash
primary_topic: automated_driving_assist
provenance:
  parser_run_id: 20260414T051134Z__dc745dc5
  source_files:
  - raw/collections/xml_kmvss/KMVSS_Att_0027_128.xml
  source_hashes:
    raw/collections/xml_kmvss/KMVSS_Att_0027_128.xml: b71721534f9d4b9153ee3a120c87da2e70731a36cb5ca10655eb5db604c4674a
  source_url: null
raw_marker: 9-60km-h-407
record_layer: knowledge
reference_articles:
- 제111조의3
regulatory_layer: technical_requirement
review_required: true
row_group_id: group-49
secondary_topics:
- automated_driving
sectno: 별표 0027
source_citation: xml_kmvss-kmvss_att_0027_128 / 9-60km-h-407
source_collection: xml_kmvss
source_url: null
statement: 9) 작동상태에 있는 자동차로유지기능은 60km/h를 초과하는 속도로 주행 중인
status: draft
subject: 부분 자율주행시스템의 안전기준(제111조의3 관련)
summary: Regulation unit `9-60km-h-407` from xml_kmvss-kmvss_att_0027_128.
title: 9) 작동상태에 있는 자동차로유지기능은 60km/h를 초과하는 속도로 주행 중인
updated: '2026-04-13'
---

# 9) 작동상태에 있는 자동차로유지기능은 60km/h를 초과하는 속도로 주행 중인

## Statement
9) 작동상태에 있는 자동차로유지기능은 60km/h를 초과하는 속도로 주행 중인

## Classification
- jurisdiction: KR
- source_collection: xml_kmvss
- regulatory_layer: technical_requirement
- phase: pre_crash
- functional_domain: crash_avoidance_and_vehicle_control
- primary_topic: automated_driving_assist
- secondary_topics: automated_driving
- browse_buckets: n/a
- legacy_domain: active_safety

## Basis
9) 작동상태에 있는 자동차로유지기능은 60km/h를 초과하는 속도로 주행 중인
자동차의 전방에서 도로를 횡단 중인 보행자를 감지하면 즉시 충돌위험을 감
소시키기 위한 형태의 운행이 시작되도록 할 것

## Authority
- clause_path: 9-60km-h-407
- source_file: raw/collections/xml_kmvss/KMVSS_Att_0027_128.xml
- source_citation: xml_kmvss-kmvss_att_0027_128 / 9-60km-h-407
- source_url: n/a
- confidence: medium

## Related Notes
- [[regulation_documents/xml_kmvss-kmvss_att_0027_128]]
- [[jurisdictions/jurisdiction-kr]]
