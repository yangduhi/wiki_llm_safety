---
aliases: []
attachment_bucket: short_value_rows
attachment_section: 부분 자율주행시스템의 안전기준(제111조의3 관련)
basis: '바) 부분 자율주행시스템 또는 자동차에 심각한 고장이 발생한 경우 운전자

  가 조향조종장치, 제동조종장치 및 가속제어장치를 조작하더라도 부분 자

  율주행시스템은 위 나)부터 라)까지에 규정된 기준과 다르게 작동할 수 있

  으나 이 경우에도 운전전환은 안전하게 이루어지도록 할 것'
browse_buckets: []
clause_path: item-171
comparison_key: 749d67c2bbcfb21ce0f89ab81696094513b19ffa
confidence: medium
created: '2026-04-13'
document_id: xml_kmvss-kmvss_att_0027_128
document_kind: attachment
effective_date: null
functional_domain:
- crash_avoidance_and_vehicle_control
id: regunit-xml_kmvss-kmvss_att_0027_128-item-171
inherits_section_context: true
inherits_subject_context: true
is_attachment: true
is_table_like_row: false
jurisdiction: KR
legacy_domain: active_safety
line_index_end: 177
line_index_start: 171
note_type: regulation_unit
page_end: null
page_start: null
parent_clause_path: 6-99
parent_clause_text: 6) 자동차로유지기능 작동 중 운전자가 조향조종장치, 제동조종장치 및 가속제
parent_title: 바) 부분 자율주행시스템 또는 자동차에 심각한 고장이 발생한 경우 운전자
phase: pre_crash
primary_topic: automated_driving_assist
provenance:
  parser_run_id: 20260414T051134Z__dc745dc5
  source_files:
  - raw/collections/xml_kmvss/KMVSS_Att_0027_128.xml
  source_hashes:
    raw/collections/xml_kmvss/KMVSS_Att_0027_128.xml: b71721534f9d4b9153ee3a120c87da2e70731a36cb5ca10655eb5db604c4674a
  source_url: null
raw_marker: item-171
record_layer: knowledge
reference_articles:
- 제111조의3
regulatory_layer: technical_requirement
review_required: true
row_group_id: group-24
secondary_topics:
- automated_driving
- braking
sectno: 별표 0027
source_citation: xml_kmvss-kmvss_att_0027_128 / item-171
source_collection: xml_kmvss
source_url: null
statement: 바) 부분 자율주행시스템 또는 자동차에 심각한 고장이 발생한 경우 운전자
status: draft
subject: 부분 자율주행시스템의 안전기준(제111조의3 관련)
summary: Regulation unit `item-171` from xml_kmvss-kmvss_att_0027_128.
title: 바) 부분 자율주행시스템 또는 자동차에 심각한 고장이 발생한 경우 운전자
updated: '2026-04-13'
---

# 바) 부분 자율주행시스템 또는 자동차에 심각한 고장이 발생한 경우 운전자

## Statement
바) 부분 자율주행시스템 또는 자동차에 심각한 고장이 발생한 경우 운전자

## Classification
- jurisdiction: KR
- source_collection: xml_kmvss
- regulatory_layer: technical_requirement
- phase: pre_crash
- functional_domain: crash_avoidance_and_vehicle_control
- primary_topic: automated_driving_assist
- secondary_topics: automated_driving, braking
- browse_buckets: n/a
- legacy_domain: active_safety

## Basis
바) 부분 자율주행시스템 또는 자동차에 심각한 고장이 발생한 경우 운전자
가 조향조종장치, 제동조종장치 및 가속제어장치를 조작하더라도 부분 자
율주행시스템은 위 나)부터 라)까지에 규정된 기준과 다르게 작동할 수 있
으나 이 경우에도 운전전환은 안전하게 이루어지도록 할 것

## Authority
- clause_path: item-171
- source_file: raw/collections/xml_kmvss/KMVSS_Att_0027_128.xml
- source_citation: xml_kmvss-kmvss_att_0027_128 / item-171
- source_url: n/a
- confidence: medium

## Related Notes
- [[regulation_documents/xml_kmvss-kmvss_att_0027_128]]
- [[jurisdictions/jurisdiction-kr]]
